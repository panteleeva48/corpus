...
@app.route('/thanks')
def thanks():
    if request.args['filename'] != '' and request.args['text'] != '':
        filename = request.args['filename']
        text = request.args['text']
        info = filename + '.txt' + '\n' + text
        writefile('data.txt',info)
        main()
...
def mystem():
    os.system("D:\\mystem.exe -cd D:\\proga\\database2\\forms.txt form_lemma.txt")

def list_lemms():
    fr = file('form_lemma.txt')
    whole_phrase = re.findall("[а-яёa-z\-]+{[а-яёa-z\|\-]+}", fr, flags=re.DOTALL)
    whole_phrase = list(set(whole_phrase))
    form_lemma_list = []
    for el in whole_phrase:
        form = re.findall("([а-яёa-z\-]+){[а-яёa-z\|\-]+}", el, flags=re.DOTALL)
        lemma = re.findall("[а-яёa-z\-]+{([а-яёa-z\|\-]+)}", el, flags=re.DOTALL)
...
def onefile():
    fr = file('data.txt')
    namefile = re.findall('(.+?\.txt)', fr, flags=re.DOTALL)
    str_base = alltables()
    str_analyse = createanalyse()
    result = str_base  + '\n' + str_analyse
    writefile(namefile[0],result)
